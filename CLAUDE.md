...
assets/css/panel.css          Account + panel styles, layered on courses.css
...
- **Student accounts are WordPress users.** `inc/auth.php` adds `/register/`,
  `/login/`, `/logout/` and `/panel/` as routes, not editor-created pages.
  Identity is the **mobile number** (`user_login` + `billing_phone` meta); email
  is optional. The auth forms post to themselves and are processed on
  `template_redirect`, the way `wp-login.php` does, so they can re-render with
  errors and keep phone numbers out of query strings and server logs.
- **The free-consultation form is gone** (29 July 2026). `zandi_handle_booking()`,
...
| Signup / login | **Built, 29 July 2026.** WordPress users, phone-first, at `/register/` `/login/` `/panel/` on the main domain. A separate `app.zandiacademy.com` was considered and deferred — one install means one login cookie and one order table. |
