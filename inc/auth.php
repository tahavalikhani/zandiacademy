...
/**
 * Registers /login/, /register/, /logout/ and /panel/.
 *
 * One rule serves all four, mirroring `zandi_course_rewrite()`. Routes rather
 * than editor-created pages, so a fresh install works without anyone having to
 * build a page tree in wp-admin first.
 *
 * @return void
 */
function zandi_account_rewrite() {
	add_rewrite_tag( '%zandi_account%', '([^&/]+)' );
	add_rewrite_rule(
		'^(' . implode( '|', zandi_account_routes() ) . ')/?$',
		'index.php?zandi_account=$matches[1]',
		'top'
	);
}
add_action( 'init', 'zandi_account_rewrite' );

/**
 * The account route the current URL asks for, or an empty string.
 *
 * The whitelist matters: the rewrite tag would otherwise accept anything and
...
function zandi_logout_url() {
	return wp_nonce_url( home_url( '/logout/' ), 'log-out' );
}
...
function zandi_account_template( $template ) {
	...
	// These routes are real pages, not the blog index.
	global $wp_query;
